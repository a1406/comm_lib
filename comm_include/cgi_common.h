...
static inline void add_statis(uint32_t appid, uint32_t open_id, uint64_t player_id, uint32_t server_id, uint32_t magic_id
	, uint32_t ext_num1, uint32_t ext_num2, uint32_t ext_num3, uint32_t ext_num4, uint32_t ext_num5
	, uint32_t ext_num6, uint32_t ext_num7, uint32_t ext_num8, uint32_t ext_num9, uint32_t ext_num10
	, const char* ext_str1=NULL, const char* ext_str2=NULL) 
{	
	char buffer[1024];
	int len = snprintf(buffer, sizeof(buffer), "insert into statis set appid=%u, open_id=%u, player_id=%.0f, server_id=%u, timestamp=now(), magic_id=%u, ext_num1=%u, ext_num2=%u"
					 ", ext_num3=%u, ext_num4=%u, ext_num5=%u, ext_num6=%u, ext_num7=%u, ext_num8=%u, ext_num9=%u, ext_num10=%u",  
			GAME_APPID, 0, (double)player_id, server_id, magic_id, ext_num1, ext_num2, ext_num3, ext_num4, ext_num5, ext_num6, ext_num7, ext_num8, ext_num9, ext_num10);

	char* p = buffer + len;
	if (ext_str1) {
		p += sprintf (p, ", ext_str1=\'%s\'", ext_str1);
	}

	if (ext_str2) {
		p += sprintf (p, ", ext_str2=\'%s\'", ext_str2);
	}
	
	unsigned long effect=0;	query(buffer, 1, &effect);	
	if (effect==0) {
		//		printf("{\"code\": 0, \"reason\": \"update mysql failed, sql: %s\"}", buffer);	
	}
}
